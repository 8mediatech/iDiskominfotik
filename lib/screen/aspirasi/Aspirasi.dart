...
class Aspirasi extends StatefulWidget {
  static String tag = '/Aspirasi';
  @override
  AspirasiState createState() => AspirasiState();
}

class AspirasiState extends State<Aspirasi> {
  String apiUrl = mBaseUrl + "get_recent_aspirasi";
  int pageNo = 2;
  ...
  void _fetchData(pageNo) async {
    if (!isLoading) {
      ...
      if (response.statusCode == 200) {
        List albumList = List();
        var resultBody;
        apiUrl = mBaseUrl + "get_recent_aspirasi&page=$pageNo";
        resultBody = jsonDecode(response.body)['aspirasi'];
  ...
  Widget appBarTitle = Text("Aspirasi",
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        title: Text("Aspirasi", style: boldTextStyle(color: db6_white, size: 24, fontFamily: fontBold)),
      ),
      body: Container(
        child: _buildList(),
      ),
