...
class PaperState extends State<Paper> {
  ...
  void _fetchData(pageNo) async {
    if (!isLoading) {
      ...
      if (response.statusCode == 200) {
        List albumList = List();
        var resultBody;
        apiUrl = mBaseUrl + "get_recent_paper&page=$pageNo";
        resultBody = jsonDecode(response.body)['paper'];
  ...
  Widget appBarTitle = Text("e-Paper",
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        title: Text("e-Paper", style: boldTextStyle(color: db6_white, size: 24, fontFamily: fontBold)),
      ),
      body: Container(
        child: _buildList(),
      ),
